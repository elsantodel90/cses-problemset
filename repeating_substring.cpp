...
int main()
{
    ...
    Notebook::bwt(N+1);
    Notebook::lcp();
    int maxIndex = int(max_element(Notebook::h, Notebook::h+N) - Notebook::h);
    int maxLength = Notebook::h[maxIndex];
    if (maxLength > 0)
    {
        int maxStart = Notebook::r[maxIndex];
        s[maxStart + maxLength] = 0;
        cout << s + maxStart << "\n";
    }
    else
        cout << "-1\n";
