...
int main()
{
    ...
    forsn(i, 1, N+1)
    {
        int suffixLength = N - Notebook::r[i];
        int newSubstringsStart = Notebook::h[i-1]+1;
        assert(newSubstringsStart <= suffixLength);
        histogram[newSubstringsStart]++;
        histogram[suffixLength+1]--;
    }
    forsn(i,1,N+1)
